...
#define EVALUATION_INTERVAL_COUNTS (EVALUATION_INTERVAL_MS \
                                    * (TIMER_CLOCK_FREQ / 1000))
...
void HAL_TIM_OC_DelayElapsedCallback(TIM_HandleTypeDef *htim)
{
  if ((htim->Instance == TIM2) && (htim->Channel == HAL_TIM_ACTIVE_CHANNEL_2))
  {
    ...
    if (!fFirstValueCaptured
        || ((lCurTime - lLastCaptureTime) > CAPTURE_TIMEOUT_MS))
    {
    ...
    if (!fSignalTimedout && (lFreq >= (TARGET_FREQ_HZ - FREQ_TOLERANCE_HZ))
