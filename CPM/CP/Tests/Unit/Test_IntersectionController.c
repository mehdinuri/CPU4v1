...
static uint8_t FakeMmuSetSafetyAction(void *ctx, MmuControlAction_t action)
{
  ...
  mmuCtx->safetyAction = action;

  return mmuCtx->setSafetyActionResult;
}
                             ...
                             OutputDriverImage_t *approved)
{
  ...
  *approved = *requested;
  ...
  if (mmuCtx->safetyAction != MMU_CONTROL_ACTION_DARK)
  {
    return 1U;
  }
...
void setUp(void)
{
  ...
  s_mmuCtx.setSafetyActionResult = 1U;
  s_mmuCtx.safetyAction = MMU_CONTROL_ACTION_NORMAL;
...
void test_controller_applies_flash_safety_action_without_forcing_all_red(void)
{
  const IntersectionRuntime_t *runtime;

  TEST_ASSERT_TRUE(IntersectionControllerSetMmuSafetyAction(
    &s_controller,
...
void test_controller_applies_dark_safety_action_to_output_filter(void)
{
  const IntersectionRuntime_t *runtime;
  ...
  TEST_ASSERT_NOT_NULL(runtime);
  TEST_ASSERT_EQUAL_UINT8(0U, runtime->mmuFlashActive);
  TEST_ASSERT_EQUAL(MMU_CONTROL_ACTION_DARK, s_mmuCtx.safetyAction);
  TEST_ASSERT_EQUAL_INT(OUTPUT_DRIVER_ASPECT_DARK,
...
int main(void)
{
  ...
  RUN_TEST(test_controller_applies_flash_safety_action_without_forcing_all_red);
  RUN_TEST(test_controller_applies_dark_safety_action_to_output_filter);
