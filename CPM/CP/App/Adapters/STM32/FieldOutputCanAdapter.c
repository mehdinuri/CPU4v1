/* App/Adapters/STM32/FieldOutputCanAdapter.c */
#include "FieldOutputCanAdapter.h"
...
#define FIELD_OUTPUT_FLASH_PERIOD_DS 5U

static FieldOutputCanAdapterCtx_t *s_registeredCtx = NULL;
...
static uint8_t FlashPhaseOn(const FieldOutputCanAdapterCtx_t *ctx)
{
  if (ctx == NULL)
  {
    return 0U;
...
static void BuildSignalImages(const FieldOutputCanAdapterCtx_t *ctx,
...
static void SendFlashConfigFrames(const uint8_t flashBits[12])
{
  ...
  payload[6] = FIELD_OUTPUT_FLASH_PERIOD_DS;
  payload[7] = 0U;
...
static void SendDateTimeFrame(FieldOutputCanAdapterCtx_t *ctx)
{
  uint8_t payload[8];
...
{
  FieldOutputCanAdapterCtx_t *adapter = (FieldOutputCanAdapterCtx_t *) ctx;
...
{
  FieldOutputCanAdapterSetConfigEpoch((FieldOutputCanAdapterCtx_t *) ctx,
                                      configEpoch);
...
IOutputDriverPort_t FieldOutputCanAdapterCreatePort(FieldOutputCanAdapterCtx_t *ctx)
{
...
{
  FieldOutputCanAdapterCtx_t *ctx = s_registeredCtx;
  uint8_t outputBits[12];
  uint8_t flashBits[12];
  ...
  if (ctx->periodicCounter >= FIELD_OUTPUT_PERIODIC_TICKS)
  {
    ...
    SendFlashConfigFrames(flashBits);
