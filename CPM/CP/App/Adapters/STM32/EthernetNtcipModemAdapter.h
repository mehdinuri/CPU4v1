/* App/Adapters/STM32/EthernetNtcipModemAdapter.h
 *
 * IModemPort concrete adapter for Ethernet NTCIP (LwIP / DHCP).
 * No serial port is used; GetBaudRate() returns 0.
 * OnInit() calls MX_LWIP_Init() to start the LwIP stack.
 */
#ifndef ETHERNET_NTCIP_MODEM_ADAPTER_H
#define ETHERNET_NTCIP_MODEM_ADAPTER_H

#include "Ports/IModemPort.h"

/* Context is empty — DHCP state is managed by LwIP internally. */
typedef struct
{
  ISerialPort_t *serialPort;
  uint8_t bPadding; /* reserved */
} EthernetNtcipModemAdapterCtx_t;

void EthernetNtcipModemAdapterInit(EthernetNtcipModemAdapterCtx_t *ctx);
IModemPort_t EthernetNtcipModemAdapterCreatePort(
  EthernetNtcipModemAdapterCtx_t *ctx);

#endif /* ETHERNET_NTCIP_MODEM_ADAPTER_H */
