...
void EthFreeRxPool(void)
{
  uint8_t bDesIdx = 0;

  for (bDesIdx = 0; bDesIdx < ETH_RX_DESC_CNT; bDesIdx++)
  {
    ETH_DMADescTypeDef *pDMARxDesc =
      (ETH_DMADescTypeDef *) heth.RxDescList.RxDesc[bDesIdx];

    if (READ_BIT(pDMARxDesc->DESC0, ETH_DMATXNDESCWBF_OWN))
    {
      struct pbuf_custom *pCustomBuf =
        (struct pbuf_custom *) ((uint8_t *) pDMARxDesc->BackupAddr0
                                - offsetof(
                                  RxBuff_t,
                                  buff));

      LWIP_MEMPOOL_FREE(RX_POOL, pCustomBuf);
    }
  }
...
void EthFreeTxPbuf(void)
{
  uint8_t bDesIdx = 0;

  ETH_TxDescListTypeDef *pDMATxDescList = &heth.TxDescList;

  HAL_ETH_ReleaseTxPacket(&heth);

  for (bDesIdx = 0; bDesIdx < ETH_TX_DESC_CNT; bDesIdx++)
  {
    ETH_DMADescTypeDef *pDMATxDesc =
      (ETH_DMADescTypeDef *) heth.TxDescList.TxDesc[bDesIdx];

    if ((READ_BIT(pDMATxDesc->DESC0,
                  ETH_DMATXNDESCWBF_OWN) == ETH_DMATXNDESCWBF_OWN)
        || (pDMATxDescList->PacketAddress[bDesIdx]
            !=
            NULL))
    {
      HAL_ETH_TxFreeCallback(pDMATxDescList->PacketAddress[bDesIdx]);
      pDMATxDescList->PacketAddress[bDesIdx] = NULL;
    }
  }

  pDMATxDescList->BuffersInUse = 0;
  pDMATxDescList->releaseIndex = ETH_TX_DESC_CNT - 1;
}
...
/* USER CODE BEGIN PHY_PRE_CONFIG */
  memcpy(&netif->hwaddr[0],
         &MCSGetEthernetMACAddress()->bAddress0,
