...
#define I2C_EEPROM_DEVICE_ADDR 0xA0
...
uint8_t I2CEEPROMWrite(uint32_t lAddress, const void *pvData,
                       uint32_t lDataSize)
{
  ...
  while (sBytesRemained > 0)
  {
    ...
    uint16_t sBytesInPage = I2C_EEPROM_PAGE_SIZE
                            - (sWriteAddr % I2C_EEPROM_PAGE_SIZE);
    uint16_t sBytesToWrite = (sBytesRemained
                              < sBytesInPage) ? sBytesRemained : sBytesInPage;
