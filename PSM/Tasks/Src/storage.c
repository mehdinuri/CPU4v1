...
uint8_t StorageRequestParse(tpSStorageReq pSReq)
{
	switch(pSReq->bReqId)
	{
		...
		{
			return I2CEEPROMWrite((uint32_t) pSReq->lAddress, pSReq->pvData, pSReq->lDataSize);
		}
		
		case STORAGE_REQ_EEPROM_READ:
		{
			return I2CEEPROMRead((uint32_t) pSReq->lAddress, pSReq->pvData, pSReq->lDataSize);
		}
	}
...
uint8_t StorageRequest(uint8_t bReqId, uint32_t lAddress, void* pvData, uint32_t lDataSize)
{
	...
	tpSStorageReq pSReq = (tpSStorageReq)osMemoryPoolAlloc(StorageReqsMemPoolHandle, 0);
	if (pSReq != NULL)
	{
		...
		if (osMessageQueuePut(StorageReqsQueueHandle, &pSReq, 0, 0) == osOK)
		{
			...
			{
				osThreadFlagsClear(THREAD_FLAGS_STORAGE_REQ_PROCESS_OK | THREAD_FLAGS_STORAGE_REQ_PROCESS_ERROR);
				uint32_t lFlags = osThreadFlagsWait(THREAD_FLAGS_STORAGE_REQ_PROCESS_OK | THREAD_FLAGS_STORAGE_REQ_PROCESS_ERROR, osFlagsWaitAny, osWaitForever);
...
void StorageTaskFunc(void *argument)
{
  ...
  while(pdTRUE)
  {
		if (osMessageQueueGet(StorageReqsQueueHandle, &pSReq, NULL,osWaitForever) == osOK)
		{
			...
			if (pSReq->bReqId != STORAGE_REQ_EEPROM_WRITE_ASYNCH)
			{
				if (pSReq->SThreadId != NULL)
				{
					if (fResult)
					{
						osThreadFlagsSet(pSReq->SThreadId, THREAD_FLAGS_STORAGE_REQ_PROCESS_OK);
					}
					else
					{
						osThreadFlagsSet(pSReq->SThreadId, THREAD_FLAGS_STORAGE_REQ_PROCESS_ERROR);
					}
				}
			}
