...
typedef struct
{
  uint8_t aCurLow[CURRENT_CHANNEL_COUNT];       /* bits 0..7 of each current */
  uint8_t bCurHighBitsPacked;                   /* 4 × 2-bit fields: channel i → bits (2*i)..(2*i+1) */
} tSCurrentMeasurementWire;

/**
 * @brief Pack a snapshot into the 10-bit-per-channel wire format.
 *
 *  Layout of bCurHighBitsPacked:
