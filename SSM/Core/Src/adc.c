...
{
	if (HAL_ADC_Start_DMA(&hadc1, (uint32_t *)saADC1ConvVals, SAMPLES_PER_CHANNEL_BUF_LEN * ADC1_MAX_CHANNELS) != HAL_OK)
	{
		Error_Handler();
	}
}
...
{
	if (HAL_ADC_Start_DMA(&hadc2, (uint32_t *)saADC2ConvVals, SAMPLES_PER_CHANNEL_BUF_LEN * ADC2_MAX_CHANNELS) != HAL_OK)
	{
		Error_Handler();
	}
}

static float SGCurrentRMSCalculate(uint16_t *saConvVals, uint8_t bOffset, uint8_t bStride, uint16_t sLength)
{
...
static float SGOptimalCurrentGet(float fRMSRawP, float fRMSRawH, float fScalingFactorP, float fScalingFactorH)
{
...
{
	uint16_t *psADC1ConvValPtr = (bBufferHalf == 0) ? &saADC1ConvVals[0] : &saADC1ConvVals[SAMPLES_PER_MAINS_CYCLE * ADC1_MAX_CHANNELS];
	uint16_t *psADC2ConvValPtr = (bBufferHalf == 0) ? &saADC2ConvVals[0] : &saADC2ConvVals[SAMPLES_PER_MAINS_CYCLE * ADC2_MAX_CHANNELS];
	...
	// Rank1: SG1_H
	// Rank2: SG2_H
	faSGP[SIGNAL_GROUP3_INDEX] = SGCurrentRMSCalculate(psADC1ConvValPtr, 0, ADC1_MAX_CHANNELS, SAMPLES_PER_MAINS_CYCLE);
	faSGP[SIGNAL_GROUP4_INDEX] = SGCurrentRMSCalculate(psADC1ConvValPtr, 1, ADC1_MAX_CHANNELS, SAMPLES_PER_MAINS_CYCLE);
	faSGH[SIGNAL_GROUP1_INDEX] = SGCurrentRMSCalculate(psADC1ConvValPtr, 2, ADC1_MAX_CHANNELS, SAMPLES_PER_MAINS_CYCLE);
	faSGH[SIGNAL_GROUP2_INDEX] = SGCurrentRMSCalculate(psADC1ConvValPtr, 3, ADC1_MAX_CHANNELS, SAMPLES_PER_MAINS_CYCLE);
	...
	// Rank3: SG2_P
	// Rank4: SG1_P
	faSGH[SIGNAL_GROUP3_INDEX] = SGCurrentRMSCalculate(psADC2ConvValPtr, 0, ADC2_MAX_CHANNELS, SAMPLES_PER_MAINS_CYCLE);
	faSGH[SIGNAL_GROUP4_INDEX] = SGCurrentRMSCalculate(psADC2ConvValPtr, 1, ADC2_MAX_CHANNELS, SAMPLES_PER_MAINS_CYCLE);
	faSGP[SIGNAL_GROUP2_INDEX] = SGCurrentRMSCalculate(psADC2ConvValPtr, 2, ADC2_MAX_CHANNELS, SAMPLES_PER_MAINS_CYCLE);
	faSGP[SIGNAL_GROUP1_INDEX] = SGCurrentRMSCalculate(psADC2ConvValPtr, 3, ADC2_MAX_CHANNELS, SAMPLES_PER_MAINS_CYCLE);
	
	uint8_t bSGIdx = 0;
	for (bSGIdx = 0; bSGIdx < SIGNAL_GROUPS_PER_SSM_MAX; bSGIdx++)
	{
		// Auto-Range Selection
		faSGCurrentRMS[bSGIdx] = SGOptimalCurrentGet(faSGP[bSGIdx], faSGH[bSGIdx], SCALING_FACTOR_P, SCALING_FACTOR_H) * 1000.0f;
	...
	MeasurementSGCurrentsSet(faSGCurrentRMS);
