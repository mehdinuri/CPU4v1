...
{
	tpSCanRxMsg pSReq = (tpSCanRxMsg)osMemoryPoolAlloc(CANRxReqsMemPoolHandle, 0);
	if(pSReq != NULL)
	{
...
void	CanTxRequest(FDCAN_HandleTypeDef *hfdcan, uint32_t lIDType, uint32_t lID, uint32_t lFrameType, uint32_t lBitRateSwitch, uint32_t lFDFormat, uint8_t *baData, uint8_t bDataLen)
{
	tpSCanTxMsg pSReq = (tpSCanTxMsg)osMemoryPoolAlloc(CANTxReqsMemPoolHandle, 0);
	if (pSReq != NULL)
	{
...
void	CanReceive(uint8_t * pbDest, uint8_t * pbSource, uint16_t sDataLen, uint16_t * psIndex, uint16_t sDataSize)
{
	uint16_t sByteNo = 0;
...
void	CanProcessData(uint32_t lId, uint8_t * pbData, uint8_t bDataLen)
{
	...
	switch(lId & 0xFF00)
	{
		case CAN_TX_CP_EXT_ID_DEFAULT:
		{
			...
			CanReceive((uint8_t *)&(SCPMPCommData.SPeripheralStates), pbData, bDataLen, &(SCPMPCommData.sIndex), sizeof(SCPMPCommData.SPeripheralStates));
			...
			else
			{
				...
				{
					if (sLampDimmingStateChangeDur++ > LAMP_DIMMING_STATE_CHANGE_DURATION)
					{
			...
			CanTxRequest(&hfdcan1, FDCAN_EXTENDED_ID, lAnsMsgId, FDCAN_DATA_FRAME, FDCAN_BRS_OFF, FDCAN_CLASSIC_CAN, (uint8_t *)(PeripheralStatesGet()), sizeof(tSPeripheralStates));
		}
		break;
	...
	switch(lId & 0xFF00)
	{
		...
		{
			SCPMPCommData.sIndex = 0;
			CanReceive((uint8_t *)&(SCPMPCommData.SaSignalDefs), pbData, bDataLen, &(SCPMPCommData.sIndex), sizeof(SCPMPCommData.SaSignalDefs));
		}
		break;
		...
		{
			SCPMPCommData.sIndex = 0;
			CanReceive((uint8_t *)&(SCPMPCommData.SSignalsDefined), pbData, bDataLen, &(SCPMPCommData.sIndex), sizeof(SCPMPCommData.SSignalsDefined));
		...
		{
			SCPMPCommData.sIndex = 0;
			CanReceive((uint8_t *)&(SCPMPCommData.SaSGDefs), pbData, bDataLen, &(SCPMPCommData.sIndex), sizeof(SCPMPCommData.SaSGDefs));
		}
		break;
		...
		{
			SCPMPCommData.sIndex = 0;
			CanReceive((uint8_t *)&(SCPMPCommData.SaSODefs), pbData, bDataLen, &(SCPMPCommData.sIndex), sizeof(SCPMPCommData.SaSODefs));
		}
		break;
		...
		{
			SCPMPCommData.sIndex = 0;
			CanReceive((uint8_t *)&(SCPMPCommData.SCVSDef), pbData, bDataLen, &(SCPMPCommData.sIndex), sizeof(SCPMPCommData.SCVSDef));
		}
		break;
		...
		{
			SCPMPCommData.sIndex = 0;
			CanReceive((uint8_t *)&(SCPMPCommData.SConflictsEM), pbData, bDataLen, &(SCPMPCommData.sIndex), sizeof(SCPMPCommData.SConflictsEM));
		...
		{
			SCPMPCommData.sIndex = 0;
			CanReceive((uint8_t *)&(SCPMPCommData.sFlashPerInEm), pbData, bDataLen, &(SCPMPCommData.sIndex), sizeof(SCPMPCommData.sFlashPerInEm));
	...
	CanTxRequest(&hfdcan1, FDCAN_EXTENDED_ID, lAnsMsgId, FDCAN_DATA_FRAME, FDCAN_BRS_OFF, FDCAN_CLASSIC_CAN, pbData, bDataLen);
}
	...
	switch(pRxMsg->RxHeader.IdType)
	{
		case FDCAN_EXTENDED_ID:
		{
			switch(pRxMsg->RxHeader.Identifier & 0xFF00)
			{
				...
				case CAN_TX_CP_EXT_ID_DEFAULT:
				{
					CanProcessData(pRxMsg->RxHeader.Identifier, (uint8_t *)&(pRxMsg->baData), pRxMsg->RxHeader.DataLength);
				}
				break;
			}
		}
		...
		case FDCAN_STANDARD_ID:
		{
			switch(pRxMsg->RxHeader.Identifier)
			{
				...
				{
					SetSSMVoltCurrentMeasurements(pRxMsg->RxHeader.Identifier - CAN_MID_SSM_VOLT_CURRENT_MEASUREMENTS0, pRxMsg->baData);
					SOCatchModuleActiveNowSet((pRxMsg->RxHeader.Identifier - CAN_MID_SSM_VOLT_CURRENT_MEASUREMENTS0), TRUE);
				}
				break;
				...
				{
					SetPSMVoltageMeasurements(pRxMsg->RxHeader.Identifier - CAN_MID_PSM_VOLT_MEASUREMENTS0, pRxMsg->baData);
					SOCatchModuleActiveNowSet((pRxMsg->RxHeader.Identifier - CAN_MID_PSM_VOLT_MEASUREMENTS0) + SIG_DEV_PSM_FIRST, TRUE);
				}
				break;
...
void	CANMsgSenderTaskFunc(void const * argument)
{
	...
	while(FOREVER)
	{
		if(osMessageQueueGet(CANTxReqsQueueHandle, &pSTxMsg, NULL, osWaitForever) == osOK)
		{
