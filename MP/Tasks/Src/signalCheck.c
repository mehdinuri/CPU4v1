...
static uint8_t baaYellowYellowConflictCounter[SIGNAL_GROUPS_MAX][SIGNAL_GROUPS_MAX];
static uint8_t baaYellowGreenConflictCounter[SIGNAL_GROUPS_MAX][SIGNAL_GROUPS_MAX];
...
static uint8_t	DetermineSOLampsState(uint8_t bOutput, uint16_t sNewCurrent, uint16_t sPrevCurrent)
{
	UNUSED(sPrevCurrent);
	...
	if (bOutput < SIGNAL_OUTPUTS_MAX)
	{
		...
		if (bWorkingLamps != GetSONoOfWorkingLamps(bOutput)) //if detected lamp count is not equal to expected lamp count
		{
			...
			{
				if (GetWorkingLampChangeCounter(bOutput) < GetWorkingLampChangesAcceptCount())
				{
					if (IncreaseWorkingLampChangeCounter(bOutput) == GetWorkingLampChangesAcceptCount())
					{
						uint8_t	bOwnerSG = GetSOOwner(bOutput) - 1;

						if (!LampDimmingStateChangedGet())
						{
							LogRequest(EVENT_SO_WORKING_LAMP_TOTAL_CHANGE, (bOutput + 1), bWorkingLamps, GetSONoOfWorkingLamps(bOutput));
							...
							if ((GetSOEM(bOutput) != EMERGENCY_METHOD_NONE) && (GetSONoOfWorkingLamps(bOutput) == 0))
							...
							if (bWorkingLamps < GetSONoOfLamps(bOutput))
							{
								switch (GetSOType(bOutput))
								{
									case SIGNAL_OUTPUT_TYPE_RED:
									{
										...
										if (GetSGNoOfLamps(bOwnerSG, SIGNAL_OUTPUT_TYPE_RED) == GetSGNoOfFailedLamps(bOwnerSG, SIGNAL_OUTPUT_TYPE_RED))
										{
											if(GetSGLastRedLampFailureEM(bOwnerSG))
												SetEmergencyMethodSet(GetSGLastRedLampFailureEM(bOwnerSG), GetSGOwner(bOwnerSG));

											if(SGRuntimeLampFailLastRedGet(bOwnerSG) == FALSE)
											{
												LogRequest(EVENT_SG_LAST_RED_LAMP_FAILURE, GetSGOwner(bOwnerSG) + 1, (bOwnerSG + 1), 1);
											}
										...
										else
										{
											...
											{
												if (bWorkingLamps <= GetSGRedLampFailureNumber(bOwnerSG))
												{
													if(GetSGRedLampFailureNumberEM(bOwnerSG))
														SetEmergencyMethodSet(GetSGRedLampFailureNumberEM(bOwnerSG), GetSGOwner(bOwnerSG));

													if(SGRuntimeLampFailNumberOfRedGet(bOwnerSG) == FALSE)
													{
														LogRequest(EVENT_SG_NUMBER_OF_RED_LAMPS_FAILURE, (bOwnerSG + 1), 0, 0);
													}
													SGRuntimeLampFailNumberOfRedSet(bOwnerSG, TRUE);
												}
												else
													SGRuntimeLampFailNumberOfRedSet(bOwnerSG, FALSE);
											}
										}
										SGRuntimeLampFailRedSet(bOwnerSG, TRUE);
										LogRequest(EVENT_SG_RED_LAMP_FAILURE, (bOwnerSG + 1), 0, 0);
									}
									break;
									...
									case SIGNAL_OUTPUT_TYPE_YELLOW:
									{
										...
										LogRequest(EVENT_SG_YELLOW_LAMP_FAILURE, (bOwnerSG + 1), 0, 0);
									}
									break;
									...
									case SIGNAL_OUTPUT_TYPE_GREEN:
									{
										...
										LogRequest(EVENT_SG_GREEN_LAMP_FAILURE, (bOwnerSG + 1), 0, 0);
									}
									break;
								}
								...
								else
								{
									switch (GetSOType(bOutput))
									{
										case SIGNAL_OUTPUT_TYPE_RED:
										{
											...
											{
												LogRequest(EVENT_SG_ALL_RED_LAMPS_BROKEN, (bOwnerSG + 1), 0, 0);
										...
										case SIGNAL_OUTPUT_TYPE_YELLOW:
										{
											...
											{
												LogRequest(EVENT_SG_ALL_YELLOW_LAMPS_BROKEN, (bOwnerSG + 1), 0, 0);
										...
										case SIGNAL_OUTPUT_TYPE_GREEN:
										{
											...
											{
												LogRequest(EVENT_SG_ALL_GREEN_LAMPS_BROKEN, (bOwnerSG + 1), 0, 0);
							...
							else if (bWorkingLamps == GetSONoOfLamps(bOutput)) //if detected Lamp count is equal to expected Lamp count
							{
								...
								switch (GetSOType(bOutput))
								{
									case SIGNAL_OUTPUT_TYPE_RED:
									{
										...
										{
											LogRequest(EVENT_SG_ALL_RED_LAMPS_SAFE, (bOwnerSG + 1), 0, 0);
										}
									...
									case SIGNAL_OUTPUT_TYPE_YELLOW:
									{
										...
										{
											LogRequest(EVENT_SG_ALL_YELLOW_LAMPS_SAFE, (bOwnerSG + 1), 0, 0);
										}
									...
									case SIGNAL_OUTPUT_TYPE_GREEN:
									{
										...
										{
											LogRequest(EVENT_SG_ALL_GREEN_LAMPS_SAFE, (bOwnerSG + 1), 0, 0);
										}
...
{
	if (ConflictingSG(bSGNo1, bSGNo2))
	{
		if ((baDisplayedVoltages[bSGNo1] & SIGNAL_OUTPUT_TYPE_YELLOW) && (baDisplayedVoltages[bSGNo2] & SIGNAL_OUTPUT_TYPE_YELLOW))
		{
			...
			if(baaYellowYellowConflictCounter[bSGNo1][bSGNo2] >= MIN_CONFLICT)
			{
				...
				if(GetYYConflictState(bSGNo1, bSGNo2) == FALSE || GetYYConflictState(bSGNo2, bSGNo1) == FALSE)
				{
					LogRequest(EVENT_YELLOW_YELLOW_CONFLICT, (bSGNo1 + 1), (bSGNo2 + 1), 0);
				}
				SetYYConflictState(bSGNo1, bSGNo2, TRUE);
				SetYYConflictState(bSGNo2, bSGNo1, TRUE);
			}
		}
		if (((baDisplayedVoltages[bSGNo1] & SIGNAL_OUTPUT_TYPE_YELLOW) && (baDisplayedVoltages[bSGNo2] & SIGNAL_OUTPUT_TYPE_GREEN)) || 
				((baDisplayedVoltages[bSGNo1] & SIGNAL_OUTPUT_TYPE_GREEN)  && (baDisplayedVoltages[bSGNo2] & SIGNAL_OUTPUT_TYPE_YELLOW)))
		{
			...
			if(baaYellowGreenConflictCounter[bSGNo1][bSGNo2] >= MIN_CONFLICT)
			{
				...
				if(GetYGConflictState(bSGNo1, bSGNo2) == FALSE || GetYGConflictState(bSGNo2, bSGNo1) == FALSE)
				{
					LogRequest(EVENT_YELLOW_GREEN_CONFLICT, (bSGNo1 + 1), (bSGNo2 + 1), 0);
				}
				SetYGConflictState(bSGNo1, bSGNo2, TRUE);
				SetYGConflictState(bSGNo2, bSGNo1, TRUE);
			}
		}
		if ((baDisplayedVoltages[bSGNo1] & SIGNAL_OUTPUT_TYPE_GREEN) && (baDisplayedVoltages[bSGNo2] & SIGNAL_OUTPUT_TYPE_GREEN))
		{
			...
			if(baaGreenGreenConflictCounter[bSGNo1][bSGNo2] >= MIN_CONFLICT)
			{
				...
				if(GetGGConflictState(bSGNo1, bSGNo2) == FALSE || GetGGConflictState(bSGNo2, bSGNo1) == FALSE)
				{
...
{
	memset(baaDisplayedVSAssignedVoltages, 0xff, sizeof(baaDisplayedVSAssignedVoltages));	//assign 0xff to displayed signals so that if DARK is observed as first signal, it is recorded too
	memset(baaYellowYellowConflictCounter, 0, sizeof(baaYellowYellowConflictCounter));
	memset(baaYellowGreenConflictCounter, 0, sizeof(baaYellowGreenConflictCounter));
	...
	memset(baFirstInvalidSignalSequences, 0, sizeof(baFirstInvalidSignalSequences));
	memset(baSecondInvalidSignalSequences, 0, sizeof(baSecondInvalidSignalSequences));
}
...
void SignalCheckTaskFunc(void const * argument)
{
	...
	while (FOREVER)
	{
		if(osMessageQueueGet(NewMeasurementsQueueHandle, &pSNewMeasurements, NULL, 1000) == osOK)
		{
			if(CPCommStateGet())
			{
				...
				if (SOCatchIsCardActive(SIG_DEV_PSM_0) || SOCatchIsCardActive(SIG_DEV_PSM_1))
				{
					...
					if (GetFrequency() < FREQUENCY_VALUE_LOWER_BOUND)
					{
						...
						if (IncrementEventCounter(0, EC_FLB))
						{
							if (GetFrequencyState() != EVENT_FREQUENCY_VALUE_LOWER_BOUND)
							{
								...
								LogRequest(EVENT_FREQUENCY_VALUE_LOWER_BOUND, 0, GetFrequency(), FREQUENCY_VALUE_LOWER_BOUND);
							}
						}
					}
					else
						...
						if (GetFrequency() > FREQUENCY_VALUE_UPPER_BOUND)
						{
							...
							if (IncrementEventCounter(0, EC_FUB))
							{
								if (GetFrequencyState() != EVENT_FREQUENCY_VALUE_UPPER_BOUND)
								{
									...
									LogRequest(EVENT_FREQUENCY_VALUE_UPPER_BOUND, 0, GetFrequency(), FREQUENCY_VALUE_UPPER_BOUND);
								}
							}
						}
						else
							...
							if (IncrementEventCounter(0, EC_FN))
							{
								...
								switch (GetFrequencyState())
								{
									...
									case EVENT_FREQUENCY_VALUE_LOWER_BOUND:
										if (GetFrequency() > (FREQUENCY_VALUE_LOWER_BOUND + FREQUENCY_VALUE_HYSTERESIS))
											fSwitchToNormalState = TRUE;
										break;
		
									case EVENT_FREQUENCY_VALUE_UPPER_BOUND:
										if (GetFrequency() < (FREQUENCY_VALUE_UPPER_BOUND - FREQUENCY_VALUE_HYSTERESIS))
											fSwitchToNormalState = TRUE;
										break;
								...
								if (fSwitchToNormalState)
								{
									...
									LogRequest(EVENT_FREQUENCY_VALUE_NORMAL, 0, GetFrequency(), 0);
								}
								else
									InitEventCounters(0, EC_FN);								
							}
					...
					if (GetVoltage() < VOLTAGE_VALUE_LOWER_BOUND)
					{
						...
						if (IncrementEventCounter(0, EC_VLB))
						{
							if (GetVoltageState() != EVENT_VOLTAGE_VALUE_LOWER_BOUND)
							{
								...
								LogRequest(EVENT_VOLTAGE_VALUE_LOWER_BOUND, 0, GetVoltage() * 0.73029, VOLTAGE_VALUE_LOWER_BOUND * 0.73029); //For Display Network Voltage in Real AC Value.
							}
						}
					}
					else
						...
						if (GetVoltage() > VOLTAGE_VALUE_UPPER_BOUND)
						{
							...
							if (IncrementEventCounter(0, EC_VUB))
							{
								if (GetVoltageState() != EVENT_VOLTAGE_VALUE_UPPER_BOUND)
								{
									...
									LogRequest(EVENT_VOLTAGE_VALUE_UPPER_BOUND, 0, GetVoltage()* 0.73029, VOLTAGE_VALUE_UPPER_BOUND * 0.73029); //For Display Network Voltage in Real AC Value.
								}
							}
						}
						else
							...
							if (IncrementEventCounter(0, EC_VN))
							{
								...
								switch (GetVoltageState())
								{
									...
									case EVENT_VOLTAGE_VALUE_LOWER_BOUND:
										if (GetVoltage() > (VOLTAGE_VALUE_LOWER_BOUND+VOLTAGE_VALUE_HYSTERESIS))
											fSwitchToNormalVoltageState = TRUE;
										break;
		
									case EVENT_VOLTAGE_VALUE_UPPER_BOUND:
										if (GetVoltage() < (VOLTAGE_VALUE_UPPER_BOUND-VOLTAGE_VALUE_HYSTERESIS))
											fSwitchToNormalVoltageState = TRUE;
										break;
								...
								if (fSwitchToNormalVoltageState)
								{
									...
									LogRequest(EVENT_VOLTAGE_VALUE_NORMAL, 0, GetVoltage()* 0.73029, 0); //For Display Network Voltage in Real AC Value.
								} 
								else
									InitEventCounters(0, EC_VN);								
							}
				...
				for (bSGNo = 0; bSGNo < bSGTotal; bSGNo++)
				{
					...
					while (bSONo)
					{
						...
						if (SOCatchIsCardActive((bSONo / SIGNAL_OUTPUTS_PER_SSM)))
						{
							...
							uint8_t	bOutputInGruoup = (bSONo % SIGNAL_OUTPUTS_PER_CURRENT_GROUP);
							uint8_t	fSOExpectedVoltage, fSO2ExpectedVoltage, fSO3ExpectedVoltage;
							uint8_t	fSOMeasuredVoltage, fSO2MeasuredVoltage, fSO3MeasuredVoltage;
							...
							//voltages
							fSOExpectedVoltage	= pSNewMeasurements->SaSORuntimeFlags[bSONo].fSwitchPrevious;
							fSOMeasuredVoltage	= pSNewMeasurements->SaSORuntimeFlags[bSONo].fVoltage;

							fSO2ExpectedVoltage	= pSNewMeasurements->SaSORuntimeFlags[bOutput2].fSwitchPrevious;
							fSO2MeasuredVoltage	= pSNewMeasurements->SaSORuntimeFlags[bOutput2].fVoltage;

							fSO3ExpectedVoltage	= pSNewMeasurements->SaSORuntimeFlags[bOutput3].fSwitchPrevious;
							fSO3MeasuredVoltage	= pSNewMeasurements->SaSORuntimeFlags[bOutput3].fVoltage;

							//currents
							bCurrentGroupNo		= (bSONo / SIGNAL_OUTPUTS_PER_CURRENT_GROUP);
							sCurrentMeasured	= pSNewMeasurements->SaSGCurrents[bCurrentGroupNo].sNow;
							sPrevCurrent			= pSNewMeasurements->SaSGCurrents[bCurrentGroupNo].sPrev;
							
							if (fSOExpectedVoltage == FALSE) // Switch Off
							{
								InitEventCounters(bSONo, EC_SOC | EC_SOS_NFF | EC_SOS_NFN | EC_SOS_NN | EC_SOS_NNF_FIR | EC_SOS_NNF_SEC | EC_VSF_SEC);
								...
								{
									InitEventCounters(bSONo, EC_LDE_FIR | EC_LDE_SEC | EC_SSC_SEC | EC_SOS_FN);
									...
									{
										if ((fSO2ExpectedVoltage == FALSE) && (fSO3ExpectedVoltage == FALSE))
										{
											...
											if(SORuntimeFailShortCircuitFirstGet(bSONo) == FALSE)
											{
												...
												{
													LogRequest(EVENT_SO_SWITCH_SHORT_CIRCUIT, (bSONo + 1), sCurrentMeasured, 1);
											...
											if(SORuntimeVoltageSensorFailureFirstGet(bSONo) == FALSE)
											{
												...
												{
													LogRequest(EVENT_SO_VOLTAGE_SENSOR_FAILURE, (bSONo + 1), 1, 0);
										...
										{
											InitEventCounters(bSONo, EC_SSC_FIR | EC_VSF_FIR | EC_SOS_FFN);
								...
								{
									InitEventCounters(bSONo, EC_SSC_FIR | EC_VSF_FIR | EC_SOS_FF | EC_SOS_FFN);
									...
									if (sCurrentMeasured < SO_MIN_POWER)
									{
										...
										{
											uint8_t	bExpectedVoltages = fSOExpectedVoltage | (fSO2ExpectedVoltage << 1) | (fSO3ExpectedVoltage << 2);
											uint8_t	bMeasuredVoltages = fSOMeasuredVoltage | (fSO2MeasuredVoltage << 1) | (fSO3MeasuredVoltage << 2);

											if (IncrementEventCounter(bSONo, EC_LDE_FIR))
											{
												LogRequest(EVENT_SO_LAMPS_DRIVEN_EXTERNALLY, (bSONo + 1), bExpectedVoltages, bMeasuredVoltages);
									...
									else
									{
										...
										if ((fSO2ExpectedVoltage == FALSE) && (fSO2MeasuredVoltage == FALSE) && 
											(fSO3ExpectedVoltage == FALSE) && (fSO3MeasuredVoltage == FALSE))
										{
											...
											if(SORuntimeFailShortCircuitSecondGet(bSONo) == FALSE)
											{
												...
												{
													LogRequest(EVENT_SO_SWITCH_SHORT_CIRCUIT, (bSONo + 1), sCurrentMeasured, 2);
													...
													SORuntimeFailShortCircuitSecondSet(bSONo, TRUE);
															...
															((fSO3ExpectedVoltage) && (fSO3MeasuredVoltage)))
										{
											...
											if(SORuntimeFailDrivenExternallySecondGet(bSONo) == FALSE)
											{
												if (IncrementEventCounter(bSONo, EC_LDE_SEC))
												{
													LogRequest(EVENT_SO_LAMPS_DRIVEN_EXTERNALLY, (bSONo + 1), 0, 0);
							...
							{
								InitEventCounters(bSONo, EC_LDE_FIR | EC_LDE_SEC | EC_SSC_FIR | EC_SSC_SEC | EC_VSF_FIR | EC_SOS_FF | EC_SOS_FFN | EC_SOS_FN);
								...
								{
									InitEventCounters(bSONo, EC_SOS_NN | EC_SOS_NNF_FIR | EC_SOS_NNF_SEC);
									...
									if (sCurrentMeasured < SO_MIN_POWER)//if there is no current
									{
										...
										if(SORuntimeFailOpenCircuitGet(bSONo) == FALSE)
										{
											...
											{
												LogRequest(EVENT_SO_SWITCH_OPEN_CIRCUIT, (bSONo + 1), 0, 0);		// switch open-circuit
									...
									else // if there is current
									{
										...
										if(SORuntimeVoltageSensorFailureSecondGet(bSONo) == FALSE)
										{
											...
											{
												LogRequest(EVENT_SO_VOLTAGE_SENSOR_FAILURE, (bSONo + 1), 2, sCurrentMeasured);	// voltage sensor circuit is broken
								...
								{
									InitEventCounters(bSONo, EC_SOC | EC_VSF_SEC | EC_SOS_NFF | EC_SOS_NFN);
									/* Commented on Mr. Suha's request for emergent Kastamonu version upgrade */
									//SORuntimeFailOpenCircuitSet(bSONo, FALSE);
									//SORuntimeVoltageSensorFailureSecondSet(bSONo, FALSE);
										...
										(fSO3ExpectedVoltage == FALSE) && (fSO3MeasuredVoltage == FALSE))
									{
										...
										if ((RecordingPower(bSONo) == FALSE) && !LampDimmingStateChangedGet())//if power record is not done before
										{
											// accept lamps as on if currently recording
											if ((GetStartRecord(bSONo) == FALSE) && (sCurrentMeasured < GetSOPower(bSONo)))		// get sample if record didn't start
											{
											...
											else
											{
												...
												else 
												{
													...
													if (SORuntimePowerRecordSampleGet(bSONo)>= MIN_CURRENT_RECORD_SAMPLES_BEFORE_START)
													{
										...
										{
											DetermineSOLampsState(bSONo, sCurrentMeasured, sPrevCurrent);
										}
									}
									else
									{
				...
				for (bSGNo = 0; bSGNo < bSGTotal; bSGNo++)
				{
					...
					if (fSafeControl)
					{
						...
						if (baaDisplayedVSAssignedVoltages[bSGNo][bAssignedVoltages] != baDisplayedVoltages[bSGNo])
						{
							LogRequest(EVENT_SIGNAL_AT_SG, (bSGNo + 1), bAssignedVoltages, baDisplayedVoltages[bSGNo]);
						}
		
						//update displayed signal record for this assignment
						baaDisplayedVSAssignedVoltages[bSGNo][bAssignedVoltages] = baDisplayedVoltages[bSGNo];
						...
						if (baDisplayedVoltages[bSGNo] != baPreviousVoltages[bSGNo])	//if displayed signal has started just now
						{
							...
							if (VoltagesValid(bSGNo, baDisplayedVoltages[bSGNo]) == FALSE)
							{
								...
								{
									LogRequest(EVENT_INVALID_SIGNAL, (bSGNo + 1), baDisplayedVoltages[bSGNo], 0);
								}
							...
							if (VoltagesCanFollowVoltages(baPreviousVoltages[bSGNo], baDisplayedVoltages[bSGNo]) == FALSE)
							{
								if(GetInvalidSignalSequenceEM())
									SetEmergencyMethodSet(GetInvalidSignalSequenceEM(), bSetNo);
								
								if(baFirstInvalidSignalSequences[bSGNo] != baPreviousVoltages[bSGNo] && baSecondInvalidSignalSequences[bSGNo] != baDisplayedVoltages[bSGNo])
								{
									LogRequest(EVENT_INVALID_SIGNAL_SEQUENCE, (bSGNo + 1), baPreviousVoltages[bSGNo], baDisplayedVoltages[bSGNo]);
								}
								
								baFirstInvalidSignalSequences[bSGNo] = baPreviousVoltages[bSGNo];
								baSecondInvalidSignalSequences[bSGNo] = baDisplayedVoltages[bSGNo];
							}
						}
