...
{
	return ((SModuleActiveness.lCurrentActiveDevices & laValue2Bit[bModuleNumber]) >> bModuleNumber);
}
...
{
	return ((SModuleActiveness.lPrevActiveDevices & laValue2Bit[bModuleNumber]) >> bModuleNumber);
}
			...
			if(IsSSMValid(bModuleNo) == TRUE)
			{
				if(ModuleActiveNowGet(bModuleNo))
				{
					...
					if(!(ModuleActivePrevGet(bModuleNo)))
					{
						...
						LogRequest(EVENT_MODULE_RESPONDS, (bModuleNo + SIG_DEV_SSM_1), 0, 0);
					}
				}
				else
				{
					...
					if(SModuleActiveness.bSSMEventCntr[bModuleNo] >= SSM_MODULE_RESPONSE_DURATION)
					{
						SModuleActiveness.bSSMEventCntr[bModuleNo] = 0;

						ModuleIsActive(bModuleNo, FALSE);

						LogRequest(EVENT_MODULE_MISSING, (bModuleNo + SIG_DEV_SSM_1), 0, SSM_MODULE_RESPONSE_DURATION * 10);
		...
		else
		{
			...
			else
			{
				if((ModuleActivePrevGet(bModuleNo)))
				{
					...
					if(SModuleActiveness.bPSMEventCntr[bModuleNo - SIG_DEV_PSM_FIRST] >= SSM_MODULE_RESPONSE_DURATION)
					{
						ModuleIsActive(bModuleNo, FALSE);

						LogRequest(EVENT_MODULE_MISSING, bModuleNo, 0, SSM_MODULE_RESPONSE_DURATION * 10);
...
void SOCatchTaskFunc(void const * argument)
{
	...
	while(FOREVER)
	{
		...
		if(CPCommStateGet())
		{
			...
			{
				tpSNewMeasurements pSMeasurements = (tpSNewMeasurements)osMemoryPoolAlloc(NewMeasurementsMemPoolHandle, osWaitForever);
				if (pSMeasurements != NULL)
				{
					...
					if (osMessageQueuePut(NewMeasurementsQueueHandle, &pSMeasurements, 0, 0) != osOK)
					{
			...
			{
				CanTxRequest(&hfdcan1, FDCAN_EXTENDED_ID, CAN_TX_MP_EXT_ID_EVENT, FDCAN_DATA_FRAME, FDCAN_BRS_OFF, FDCAN_CLASSIC_CAN, (uint8_t *)pSNewEvent, sizeof(tSEvent));
