...
void CheckThreadStackUsage(void)
{
	...
	uxNumberOfTasks = uxTaskGetSystemState( pxTaskStatusArray, APP_THREADS_MAX, &ulTotalRunTime);
	...
	{
		printf("%-*s\t%lu\n", configMAX_TASK_NAME_LEN, pxTaskStatusArray[x].pcTaskName, (unsigned long)pxTaskStatusArray[x].usStackHighWaterMark);

		if (pxTaskStatusArray[x].usStackHighWaterMark == 0) 
		{
			 printf("  WARNING: Task '%s' stack potentially overflowed or critically low!\n", pxTaskStatusArray[x].pcTaskName);
		} 
		else if (pxTaskStatusArray[x].usStackHighWaterMark < 32) 
		{
			printf("\tINFO: Task '%s' stack usage high (low watermark: %lu words).\n", pxTaskStatusArray[x].pcTaskName, (unsigned long)pxTaskStatusArray[x].usStackHighWaterMark);
		}
	}
...
static void	ModuleVersionTxReq(void)
{
	...
	CanTxRequest(&hfdcan1, FDCAN_EXTENDED_ID, CAN_TX_MP_EXT_ID_MP_VERSION, FDCAN_DATA_FRAME, FDCAN_BRS_OFF, FDCAN_CLASSIC_CAN, (uint8_t *)&(sMPVersion), sizeof(sMPVersion));
}

static void ModuleRestartedTxReq(void)
{
	CanTxRequest(&hfdcan1, FDCAN_EXTENDED_ID, CAN_TX_MP_EXT_ID_RESET, FDCAN_DATA_FRAME, FDCAN_BRS_OFF, FDCAN_CLASSIC_CAN, NULL, 0);
}
